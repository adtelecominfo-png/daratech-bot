...
async function isOwnerOrSudo(senderId, sock = null, chatId = null) {
    ...
    // Check if sender ID matches bot's own user ID (owner using same account)
    if (sock && sock.user?.id) {
        const botNumeric = sock.user.id.split(':')[0].split('@')[0];
        const senderNumeric = senderId.split(':')[0].split('@')[0];
        if (botNumeric === senderNumeric) return true;
        if (senderId === sock.user.id) return true;
    }
