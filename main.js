...
const { afkCommand, handleAfk } = require('./commands/afk');
...
const technewsCommand = require('./commands/technews');
...
// Channel tag removed -- messages are sent without any forwarded-channel context.
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        if (!userMessage.startsWith('.')) {
            ...
            // AFK check (clears sender's AFK, warns anyone mentioning/replying
            // to an AFK user) -- runs in both DMs and groups.
            await handleAfk(sock, chatId, message, senderId);
            return;
        }
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.afk'):
                {
                    // Use rawText (preserves original casing) for the reason,
                    // not the lowercased userMessage used for command matching.
                    const reasonText = rawText.slice(rawText.toLowerCase().indexOf('.afk') + '.afk'.length).trim();
                    await afkCommand(sock, chatId, message, senderId, reasonText);
                    commandExecuted = true;
                }
                break;
            case userMessage.startsWith('.poll'):
                {
                    const pollText = rawText.slice(rawText.toLowerCase().indexOf('.poll') + '.poll'.length).trim();
                    await pollCommand(sock, chatId, message, pollText);
                    commandExecuted = true;
                }
                break;
            case userMessage.startsWith('.remind'):
                {
                    const remindText = rawText.slice(rawText.toLowerCase().indexOf('.remind') + '.remind'.length).trim();
                    await reminderCommand(sock, chatId, message, senderId, remindText);
                    commandExecuted = true;
                }
                break;
            case (userMessage === '.calc' || userMessage.startsWith('.calc ')):
                {
                    const calcExpr = rawText.slice(rawText.toLowerCase().indexOf('.calc') + '.calc'.length).trim();
                    await calculatorCommand(sock, chatId, message, calcExpr);
                    commandExecuted = true;
                }
                break;
            ...
            case userMessage === '.catfact':
                ...
                break;
            case userMessage === '.technews':
                await technewsCommand(sock, chatId, message);
                break;
